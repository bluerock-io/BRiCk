...
let rec eta_reduce_pat (p : Pattern.constr_pattern) =
  let open Pattern in
  match p with
| PLambda (_, _, q) ->
  ...
  if napp > 0 then
    let r = eta_reduce_pat (CArray.last cl) in
    match r with
    | PRel 1 ->
      let lc = Array.sub cl 0 (napp - 1) in
      let u = if CArray.is_empty lc then f else PApp (f, lc) in
...
let decomp_pat p =
  ...
  let rec decrec acc = function
    ...
    | PProj (p, c) ->
      ...
      let params = CList.make (Names.Projection.npars p) hole in
      (PRef (Names.GlobRef.ConstRef (Names.Projection.constant p)), params @ c :: acc)
    | c -> (c,acc)
  in
