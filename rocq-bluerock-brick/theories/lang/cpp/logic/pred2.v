...
Module Type CPP_LOGIC
  ...
  Context `{cpp_logic}.
    ...
    Axiom provides_storage_same_address : forall `{genv} storage_ptr obj_ptr ty,
  ...
  Context `{cpp_logic}.
    ...
    Axiom type_ptr_valid_plus_one : forall `{genv} ty p,
      type_ptr ty p |-- valid_ptr (p ,, o_sub ty 1).
    ...
    Axiom same_address_eq_null : forall `{genv} p tv,
      _valid_ptr tv p |--
...
Module Type VALID_PTR_AXIOMS
  ...
  Section with_cpp.
    ...
    Axiom _valid_ptr_nullptr_sub_false : forall vt ty (i : Z) (_ : i <> 0),
      _valid_ptr vt (nullptr ,, o_sub ty i) |-- False.
    (*
    ...
    *)
    Axiom strict_valid_ptr_sub : ∀ (i j k : Z) p ty vt1 vt2,
      (i <= j < k)%Z ->
      _valid_ptr vt1 (p ,, o_sub ty i) |--
      _valid_ptr vt2 (p ,, o_sub ty k) -* strict_valid_ptr (p ,, o_sub ty j).
    ...
    *)
    Axiom strict_valid_ptr_field_sub : ∀ (p : ptr) ty (i : Z) f vt,
      (0 < i)%Z ->
      _valid_ptr vt (p ,, o_field f ,, o_sub ty i) |-- strict_valid_ptr (p ,, o_field f).

    (* TODO: can we deduce that [p] is strictly valid? *)
    Axiom _valid_ptr_field : ∀ p f vt,
      _valid_ptr vt (p ,, o_field f) |-- _valid_ptr vt p.
