...
Module Type PTRS.
  ...
  *)
  ...
  #[global] Notation "p ., o" := (_dot p (o_field o))
  ...
  Parameter o_base : forall (derived base : name), offset.
  Parameter o_derived : forall (base derived : name), offset.
   ...
   *)
  Parameter ptr_vaddr : ∀ `{genv}, ptr -> option vaddr.

  Axiom ptr_vaddr_resp_leq :
    ∀ σ1 σ2,
      genv_leq σ1 σ2 ->
   ...
   *)
  Axiom ptr_vaddr_nullptr : ∀ `{genv}, ptr_vaddr nullptr = Some 0%N.

  Axiom global_ptr_nonnull_addr : forall `{genv} tu o, ptr_vaddr (global_ptr tu o) <> Some 0%N.
  ...
  Axiom global_ptr_addr_inj : forall `{genv} tu, Inj (=) (=) (λ o, ptr_vaddr (global_ptr tu o)).
  ...
  Axiom ptr_vaddr_o_sub_eq : forall p σ ty n1 n2 sz,
    ...
    same_property ptr_vaddr (p ,, o_sub ty n1) (p ,, o_sub ty n2) ->
    n1 = n2.
  ...
  *)
  Axiom eval_o_field : forall σ n cls st,
...
Module Type PTRS_DERIVED (Import P : PTRS).
  ...
  Parameter same_address : ∀ `{genv}, ptr -> ptr -> Prop.
  Axiom same_address_eq : ∀ `{genv}, same_address = same_property ptr_vaddr.
...
Module Type PTRS_MIXIN (Import P : PTRS_INTF_MINIMAL).
  ...
  *)
  ...
  #[global] Instance same_address_dec `{genv} : RelDecision same_address.
  Proof. rewrite same_address_eq. apply _. Qed.
  #[global] Instance same_address_per `{genv} : RelationClasses.PER same_address.
  Proof. rewrite same_address_eq. apply _. Qed.
  #[global] Instance same_address_RewriteRelation `{genv} : RewriteRelation same_address := {}.

  Lemma same_address_iff `{genv} p1 p2 :
    same_address p1 p2 <-> ∃ va, ptr_vaddr p1 = Some va ∧ ptr_vaddr p2 = Some va.
  Proof. by rewrite same_address_eq same_property_iff. Qed.

  Lemma same_address_intro `{genv} p1 p2 va :
    ptr_vaddr p1 = Some va -> ptr_vaddr p2 = Some va -> same_address p1 p2.
  Proof. rewrite same_address_eq; exact: same_property_intro. Qed.

  Lemma same_address_nullptr_nullptr `{genv} : same_address nullptr nullptr.
  Proof. have ? := ptr_vaddr_nullptr. exact: same_address_intro. Qed.

  #[global] Instance ptr_vaddr_proper `{genv} :
    Proper (same_address ==> eq) ptr_vaddr.
  Proof. by intros p1 p2 (va&->&->)%same_address_iff. Qed.

  #[global] Instance ptr_vaddr_params `{genv} : Params ptr_vaddr 1 := {}.


  (** ** [same_address_bool] lemmas *)
  Definition same_address_bool `{genv} p1 p2 := bool_decide (same_address p1 p2).

  #[global] Instance same_address_bool_comm `{genv} : Comm eq same_address_bool.
  Proof. move=> p1 p2. exact: bool_decide_ext. Qed.

  Lemma same_address_bool_eq `{genv} {p1 p2 va1 va2} :
  ...
  Lemma same_address_bool_partial_reflexive `{genv} p :
    is_Some (ptr_vaddr p) ->
  ...
  Lemma same_address_o_sub_eq p σ ty n1 n2 sz :
    size_of σ ty = Some sz -> (sz > 0)%N ->
    same_address (p ,, o_sub ty n1) (p ,, o_sub ty n2) -> n1 = n2.
  ...
  the given alignment.
    *)
  Definition aligned_ptr `{genv} align p :=
    (exists va, ptr_vaddr p = Some va /\ (align | va)%N) \/ ptr_vaddr p = None.
  Definition aligned_ptr_ty `{genv} ty p :=
  ...
  Proof.
    ...
    f_equiv => align [Hal Hp]; split.
    exact: (align_of_genv_leq σ1) Hal Hg.
    move: Hp. by rewrite /aligned_ptr ptr_vaddr_resp_leq.
  Qed.
  ...
  #[global] Instance aligned_ptr_divide_mono `{genv} :
  ...
  Lemma pinned_ptr_pure_aligned_divide `{genv} va n p :
    ptr_vaddr p = Some va ->
